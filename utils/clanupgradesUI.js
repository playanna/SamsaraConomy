...
const upgradeConfig = require('./clanupgradeConfig');
...
function createAllUpgradeButtons(multipliers, balance, clanupgrades) {
  const buttons = [];

  for (const type in upgradeConfig) {
    const config = upgradeConfig[type];
    const level = clanupgrades[config.key] || 0;
...
function buildMeditationEmbed(interaction, multipliers, balance, clanupgrades) {
  const fields = [];
...
function buildBreakthroughEmbed(interaction, multipliers, level, balance, clanupgrades) {
  const fields = []; 
  const upgradeEntries = [];
